...
export async function readSseStream(
  ...
  try {
    ...
    while (true) {
      ...
      for (const line of lines) {
        if (!line.startsWith('data:')) {
          // Ignore comments and other SSE fields (id:, event:, retry:)
          continue;
        }
        ...
        try {
          ...
          // Fallback: surface as token-like payload to avoid losing data
          onData({ type: 'token', content, token: content });
        }
      }
    }
    ...
    if (buffer.length > 0 && buffer.startsWith('data:')) {
      const content = buffer.startsWith('data: ')
        ? buffer.slice(6)
        : buffer.slice(5);
      const trimmed = content.trim();
      if (trimmed === '[DONE]') {
